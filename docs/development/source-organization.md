...
Add a top-level source directory only when the first real feature needs it.
...
A non-block extension should follow the same feature-first ownership rule.
...
Create only the files and subdirectories required by the implementation. Do not introduce generic architecture in advance.
...
Keep focused TypeScript tests beside the modules they verify when a test framework exists. Use a top-level test directory only for repository-wide, integration, PHP, or end-to-end behavior that does not belong naturally beside one source module.
