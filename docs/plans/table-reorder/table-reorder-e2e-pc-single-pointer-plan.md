...
- hover 可能な PC 環境で、行ハンドルをクリックすると操作案内と有効な移動先が表示され、有効な移動先をクリックすると行順が更新されることを固定する。
...
3. 行ハンドルをクリックする。
4. `Click destination　Esc Cancel` / `移動先をクリック　Esc キャンセル` の現在の操作案内が表示されることを確認する。
5. 有効な移動先が表示されることを確認する。
6. 元と同じ位置が確定可能な移動先として表示されないことを確認する。
7. 有効な別位置をクリックする。
8. 操作案内と移動先 UI が閉じることを確認する。
9. Table の編集内容から行順が期待どおり変わったことを確認する。
...
### Phase 3: Escape キャンセルで行順を変更しないことを固定する

Outcome:

- PC で移動先選択中に Escape キーを押した場合、行順を変更せず通常操作へ戻れることを固定する。
