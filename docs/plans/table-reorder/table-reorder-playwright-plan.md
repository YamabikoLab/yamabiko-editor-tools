...
| データセット | 用途 | 内容 |
|---|---|---|
| `basicTable` | キーボード主要フロー、ポインターDnD | 通常行4行程度。各行を画面上の内容から識別できる単純なTable |
...
- キーボード操作とポインターDnDの基本フローではこのデータだけを使い、結合セルなど別の要因を持ち込まない。
...
non-iframe へ同じケースを機械的に複製するのは、環境差による回帰が確認された場合に限る。基本的なポインターDnD自体は 4.4 で iframe / non-iframe の両方を確認する。
...
5. キーボード主要フローを iframe で成立させる。
6. 同じフローを non-iframe で確認する。
7. ポインターDnD主要フローを iframe で成立させる。
8. 同じフローを non-iframe で確認する。
9. `mergedCellsTable` で `colspan` を越える安全な移動と `rowspan` 分断拒否の代表ケースを確認する。
10. 必要最小限の長距離スクロール追従ケースを iframe で追加する。
11. 重複したsetup、テストデータ、実装詳細依存を整理する。
12. 対象E2Eと既存の非破壊チェックを実行する。
