...
It is not production code and does not attempt to reproduce the previous keyboard reorder UI, announcements, merged-cell restrictions, or every accessibility behavior.
...
- Touch / hover-unavailable environments keep row handles absent during normal editing and expose a toolbar reorder mode that shows all row handles.
...
2. Move the pointer into the 32px gutter at the inline start of a body row.
...
A separate reorder mode is intentionally not required for fine-pointer environments.
...
3. Use the **並び替え** button in the block toolbar to turn reorder mode on.
4. All body-row handles and gutters appear while reorder mode is on.
5. Drag a row from its handle.
6. Turn **並び替え** off to return to the normal editing / scrolling state.
...
- PC / mouse shows only the hovered row handle and can drag directly from the gutter;
- touch / hover-unavailable normal mode has no handles and preserves normal editing / scrolling;
- touch reorder mode shows all handles and can drag from them;
...
4. Does the row push-aside animation feel stable in both input modes?
5. Does full-width Table work without special width shrinking?
6. Does scrolling during drag remain stable without handle-position tracking?
7. After drop, does Gutenberg render the committed row order without flicker or stale DOM?
8. Does the approach behave in both iframe and non-iframe editor versions you support?
